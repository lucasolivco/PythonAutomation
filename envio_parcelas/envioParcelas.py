...
def registrar_log(empresa, numero, pdf_path):
    ...
    log_file = os.path.join(script_dir, "automacao_log.txt")
    
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    try:
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"{timestamp} - Empresa: {empresa}, Número: {numero}, PDF: {pdf_path}\n\n")
        print(f"Log registrado com sucesso para {empresa}")
    except PermissionError:
        print(f"Erro de permissão ao tentar escrever no arquivo de log: {log_file}")
        print("Tente executar o script como administrador ou verificar as permissões do diretório.")
    except Exception as e:
        print(f"Erro inesperado ao registrar log: {e}")

    # Imprime o caminho do arquivo de log para debug
    print(f"Tentando criar/acessar o arquivo de log em: {log_file}")
...
caminho_excel = r'C:\Users\Canella e Santos\Documents\Envio Parcelamentos\Envio Parcelamentos\Associação empresas-número.xlsx'  # Substitua pelo caminho real da sua planilha
